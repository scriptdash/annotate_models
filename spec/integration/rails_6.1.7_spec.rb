...
describe 'Integration testing on Rails 6.1.7', if: IntegrationHelper.able_to_run?(__FILE__, RUBY_VERSION) do
  ::RAILS_6_1_APP_NAME = 'rails_6.1.7'.freeze
  ::RAILS_6_1_PROJECT_PATH = File.expand_path('../..', __dir__).freeze
  ::RAILS_6_1_APP_PATH = File.expand_path(RAILS_6_1_APP_NAME, __dir__).freeze

  let!(:git) { Git.open(RAILS_6_1_PROJECT_PATH) }
  let(:task_model) do
    patch = <<~PATCH
      +# == Schema Information
      +#
  ...
  before(:all) do
    Bundler.with_clean_env do
      Dir.chdir RAILS_6_1_APP_PATH do
        puts `bundle install`
