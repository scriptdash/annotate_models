...
describe 'Integration testing on Rails 7.0.4', if: IntegrationHelper.able_to_run?(__FILE__, RUBY_VERSION) do
  ::RAILS_7_0_APP_NAME = 'rails_7.0.4'.freeze
  ::RAILS_7_0_PROJECT_PATH = File.expand_path('../..', __dir__).freeze
  ::RAILS_7_0_APP_PATH = File.expand_path(RAILS_7_0_APP_NAME, __dir__).freeze

  let!(:git) { Git.open(RAILS_7_0_PROJECT_PATH) }
  let(:task_model) do
    patch = <<~PATCH
      +# == Schema Information
      +#
  ...
  before(:all) do
    Bundler.with_clean_env do
      Dir.chdir RAILS_7_0_APP_PATH do
        puts `bundle install`
